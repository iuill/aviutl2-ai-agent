...
mod tests {
    ...
    fn health_round_trip_uses_camel_case() {
        let health = Health {
            ...
            plugin_version: "test-version".into(),
        };
        let json = serde_json::to_string(&health).unwrap();
        assert_eq!(json, r#"{"status":"ok","pluginVersion":"test-version"}"#);
        assert_eq!(serde_json::from_str::<Health>(&json).unwrap(), health);
    }

    #[test]
    fn health_accepts_unknown_fields_for_forward_compatibility() {
        let json = r#"{"status":"ok","pluginVersion":"test-version","surprise":true}"#;
    ...
    fn phase1_responses_use_stable_field_names() {
        ...
        assert_eq!(
            ...
            r#"{"status":"ok","pluginVersion":"test-version","apiVersion":"v1","listenerAddress":"127.0.0.1:7890","processId":42}"#
        );
    ...
    fn phase1_responses_accept_unknown_fields() {
        let status = serde_json::from_str::<Status>(
            r#"{"status":"ok","pluginVersion":"test-version","apiVersion":"v1","listenerAddress":"127.0.0.1:7890","processId":42,"future":true}"#,
        )
        .unwrap();
